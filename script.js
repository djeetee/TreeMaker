...
function addRootNode() {
  ...
  const tree_container_width = tree_container_width_txt.substring(
    0,
    tree_container_width_txt.length - 2
  )
  const tree_container_height_txt = tree_container_props.height
  const tree_container_height = tree_container_height_txt.substring(
    0,
    tree_container_height_txt.length - 2
  )
  ...
  myDraggable.onMove = function (newPositionLT) {
    ...
    const positionDiff = get_position_diff(
      myTree[rootIndex].oldPosition,
      newPosition
    )
...
function setDraggablePositionOfChild(inDraggable, inParentPosition) {
  ...
  inDraggable.left = inParentPosition.x + 0 + pos.x
...
function addNode(text_content, ParentIndex) {
  ...
  let myLeaderLine = new LeaderLine(ParentNode, NewNode)
  ...
  NewDraggable.onMove = function (newPositionLT) {
    ...
    const positionDiff = get_position_diff(
      myTree[NewNodeIndex].oldPosition,
      newPosition
    )
